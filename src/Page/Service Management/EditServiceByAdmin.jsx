...
import { CircularProgress, FormControlLabel, FormGroup, Switch, Button } from '@mui/material';
...
const EditServiceByAdmin = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/service/${id}`
                );
    ...
    const handleProductSubmit = async () => {
        ...
        try {
            const res = await axios.put(`${process.env.REACT_APP_BASE_URL}/service/${id}/updateService`, ProductFormData, {
                headers: {
                    ...
                    <div className='bg-gray-50'>
                        ...
                        <div className="bg-white mx-10  flex  gap-5 ">
                            <div className="basis-[70%] flex gap-10">
                                <div className="basis-[100%] p-10">
                                    ...
                                    <div>
                                        ...
                                        <Box>
                                            <Typography variant="h6" sx={{ marginTop: "10px" }} >Uploaded Categories</Typography>
                                            <div className="p-4 flex flex-wrap">
                                                {service?.pricing.reduce((acc, curr, index) => {
                                                    const { attribute, value, price } = curr;
                                                    const attributeExists = acc.find((item) => item.attribute === attribute);
                                                    ...
                                                    if (!attributeExists) {
                                                        ...
                                                        attributeExists.subcategories.push({ value, price });
                                                    }

                                                    return acc;
                                                }, []).map((item, index) => (
                                                    <div key={index} className="mb-6 w-[200px]">
                                                        <p>Main Category:<span className="text-lg ml-2 font-semibold mb-2">{item.attribute}</span></p>
                                                        <div className="ml-4 space-y-2">
                                                            {item.subcategories.map((sub, i) => (
                                                                <span> Sub Category: <p key={i} className="text-gray-700 font-semibold">
                                                                    <span className=" font-semibold">{sub.value}:</span> ₹{sub.price}
                                                                </p> </span>
                                                            ))}
                                                        </div>
                                                    </div>
                                        ...
                                        <Typography variant="h6" sx={{ marginTop: "10px" }} >Upload more Categories</Typography>
                            ...
                            <div className="basis-[25%] max-w-[380px]  px-7 ">
                                ...
                                <div>
                                    <div className="mt-12">
                                        <p className='text-center text-xl my-8 '>Upload New Images</p>
